package com.MeshLink.android.protocol
...
enum class MessageType(val value: UByte) {
    ...
    VOICE_STREAM(0x23u), // Live real-time audio stream chunk
    CALL_REQUEST(0x24u), // Broadcasted to initiate a live audio call
    CALL_ACCEPT(0x25u), // Sent to the caller by the peer taking the call
    CALL_END(0x26u), // Hang up, or a caller turning away a second answerer
    QUERY(0x30u), // Knowledge query relayed to a peer that has a route to the Ground Station
    QUERY_RESULT(0x31u); // Answer to a QUERY, carrying scored passages and their provenance
...
data class MeshLinkPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = MeshLinkPacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            route = route,
            ttl = com.MeshLink.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
        return BinaryProtocol.encode(unsignedPacket)
    }

    companion object {
        fun fromBinaryData(data: ByteArray): MeshLinkPacket? {
...
object BinaryProtocol {
    ...
    /** Largest payload a v1 packet can describe, since its length field is 16 bits. */
    private const val MAX_V1_PAYLOAD_SIZE = 65_535
    ...
    fun encode(packet: MeshLinkPacket): ByteArray? {
        try {
            ...
            if (packet.version >= 2u.toUByte()) {
                ...
                //
                // `toShort()` wraps silently, so a 200 KB payload on a v1 packet used to write a
                // length of 3392. The receiver then read 3392 bytes of a 200 KB packet and the decode
                // failed or produced garbage — which is why private images above ~64 KB "sometimes"
                // never arrived, with no error anywhere. Returning null surfaces the problem to the
                // caller instead of putting a corrupt frame on the air.
                if (payloadDataSize > MAX_V1_PAYLOAD_SIZE) {
                    Log.e(
                        TAG,
                        "Payload of $payloadDataSize bytes exceeds the v1 limit of " +
                            "$MAX_V1_PAYLOAD_SIZE; the packet must be built as version 2 " +
                            "(type=${packet.type})",
                    )
                    return null
                }
    ...
    fun decode(data: ByteArray): MeshLinkPacket? {
     ...
     */
    private fun decodeCore(raw: ByteArray): MeshLinkPacket? {
        try {
            ...
            if (payloadLength > com.MeshLink.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length ${payloadLength} exceeds maximum allowed (${com.MeshLink.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
