...
import com.MeshLink.android.model.PrivateMessagePacket
import com.MeshLink.android.protocol.MeshLinkPacket
import com.MeshLink.android.services.SeenMessageStore
import com.MeshLink.android.ui.ChatState
import com.MeshLink.android.ui.MeshDelegateHandler
import com.MeshLink.android.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.MeshLink.android.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (!content.startsWith("MeshLink1:")) return@launch

                val base64Content = content.removePrefix("MeshLink1:")
                val packetData = base64URLDecode(base64Content) ?: return@launch
                val packet = MeshLinkPacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.MeshLink.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.MeshLink.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        ...
        when (payload.type) {
            NoisePayloadType.PRIVATE_MESSAGE -> {
                ...
                val type = if (com.MeshLink.android.model.parseLocation(pm.content) != null) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = MeshLinkFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.MeshLink.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = MeshLinkMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
