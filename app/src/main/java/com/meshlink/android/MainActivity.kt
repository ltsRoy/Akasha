...
import com.MeshLink.android.onboarding.BluetoothCheckScreen
import com.MeshLink.android.onboarding.BluetoothStatus
import com.MeshLink.android.onboarding.BluetoothStatusManager
import com.MeshLink.android.onboarding.BatteryOptimizationManager
import com.MeshLink.android.onboarding.BatteryOptimizationPreferenceManager
import com.MeshLink.android.onboarding.BatteryOptimizationScreen
import com.MeshLink.android.onboarding.BatteryOptimizationStatus
import com.MeshLink.android.onboarding.BackgroundLocationPermissionScreen
import com.MeshLink.android.onboarding.InitializationErrorScreen
import com.MeshLink.android.onboarding.InitializingScreen
import com.MeshLink.android.onboarding.LocationCheckScreen
import com.MeshLink.android.onboarding.LocationStatus
import com.MeshLink.android.onboarding.LocationStatusManager
import com.MeshLink.android.onboarding.NicknameSetupScreen
import com.MeshLink.android.onboarding.OnboardingCoordinator
import com.MeshLink.android.onboarding.OnboardingState
import com.MeshLink.android.onboarding.PermissionExplanationScreen
import com.MeshLink.android.onboarding.PermissionManager
import com.MeshLink.android.ui.ChatScreen
import com.MeshLink.android.ui.ChatViewModel
import com.MeshLink.android.ui.home.AkashaHomeScreen
import com.MeshLink.android.ui.OrientationAwareActivity
import com.MeshLink.android.ui.theme.MeshLinkTheme
import com.MeshLink.android.nostr.PoWPreferenceManager
...
class MainActivity : OrientationAwareActivity() {
    ...
    private val forceFinishReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context, intent: android.content.Intent) {
            if (intent.action == com.MeshLink.android.util.AppConstants.UI.ACTION_FORCE_FINISH) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = android.content.IntentFilter(com.MeshLink.android.util.AppConstants.UI.ACTION_FORCE_FINISH)
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            registerReceiver(
                forceFinishReceiver,
                filter,
                com.MeshLink.android.util.AppConstants.UI.PERMISSION_FORCE_FINISH,
                null,
        ...
        com.MeshLink.android.service.AppShutdownCoordinator.cancelPendingShutdown()
        ...
        try { com.MeshLink.android.service.MeshForegroundService.start(applicationContext) } catch (_: Exception) { }
        meshService = com.MeshLink.android.service.MeshServiceHolder.getOrCreate(applicationContext)
    ...
    private fun OnboardingFlowScreen(modifier: Modifier = Modifier) {
        ...
        when (onboardingState) {
            ...
            OnboardingState.CHECKING, OnboardingState.INITIALIZING, OnboardingState.COMPLETE -> {
                ...
                // First run: require a real display name before anything else. Placed after the
                // permission/Bluetooth onboarding so the mesh is already up — the announce that
                // follows setNickname then actually reaches nearby peers.
                val needsNickname by chatViewModel.needsNicknameSetup.collectAsState()

                // Akasha lands on the map/orb home; the full chat timeline is one tap away.
                var showChat by rememberSaveable { mutableStateOf(false) }

                when {
                    needsNickname -> NicknameSetupScreen(
                        onConfirm = { chatViewModel.setNickname(it) },
                        validate = { chatViewModel.validateNickname(it) },
                        modifier = modifier,
                    )
                    ...
                    else -> AkashaHomeScreen(
                        viewModel = chatViewModel,
                        onOpenChat = { showChat = true },
                        onOpenPeers = { chatViewModel.showMeshPeerList() },
                        modifier = modifier,
                    )
                }
            }
        ...
        lifecycleScope.launch {
            ...
            if (permissionManager.isFirstTimeLaunch()) {
                ...
                if (permissionManager.needsBackgroundLocationPermission() &&
                    ...
                    !com.MeshLink.android.onboarding.BackgroundLocationPreferenceManager.isSkipped(this@MainActivity)
                ) {
                    mainViewModel.updateOnboardingState(OnboardingState.BACKGROUND_LOCATION_EXPLANATION)
                } else {
    ...
    private fun handleOnboardingComplete() {
        ...
        when {
            ...
            currentBatteryOptimizationStatus == BatteryOptimizationStatus.ENABLED && !com.MeshLink.android.onboarding.BatteryOptimizationPreferenceManager.isSkipped(this) -> {
    ...
    private fun initializeApp() {
        ...
        lifecycleScope.launch {
            try {
                ...
                com.MeshLink.android.nostr.LocationNotesInitializer.initialize(this@MainActivity)
    ...
    private fun handleNotificationIntent(intent: Intent) {
        val shouldOpenPrivateChat = intent.getBooleanExtra(
            com.MeshLink.android.ui.NotificationManager.EXTRA_OPEN_PRIVATE_CHAT, 
            false
        )
        
        val shouldOpenGeohashChat = intent.getBooleanExtra(
            com.MeshLink.android.ui.NotificationManager.EXTRA_OPEN_GEOHASH_CHAT,
            false
        )
        
        when {
            shouldOpenPrivateChat -> {
                val peerID = intent.getStringExtra(com.MeshLink.android.ui.NotificationManager.EXTRA_PEER_ID)
                val senderNickname = intent.getStringExtra(com.MeshLink.android.ui.NotificationManager.EXTRA_SENDER_NICKNAME)
            ...
            shouldOpenGeohashChat -> {
                val geohash = intent.getStringExtra(com.MeshLink.android.ui.NotificationManager.EXTRA_GEOHASH)
                ...
                if (geohash != null) {
                    ...
                    val level = when (geohash.length) {
                        7 -> com.MeshLink.android.geohash.GeohashChannelLevel.BLOCK
                        6 -> com.MeshLink.android.geohash.GeohashChannelLevel.NEIGHBORHOOD
                        5 -> com.MeshLink.android.geohash.GeohashChannelLevel.CITY
                        4 -> com.MeshLink.android.geohash.GeohashChannelLevel.PROVINCE
                        2 -> com.MeshLink.android.geohash.GeohashChannelLevel.REGION
                        else -> com.MeshLink.android.geohash.GeohashChannelLevel.CITY // Default fallback
                    }
                    val geohashChannel = com.MeshLink.android.geohash.GeohashChannel(level, geohash)
                    val channelId = com.MeshLink.android.geohash.ChannelID.Location(geohashChannel)
    ...
    private fun handleVerificationIntent(intent: Intent) {
        ...
        if (uri.scheme != "MeshLink" || uri.host != "verify") return
    ...
    private fun handleRelayIntent(intent: Intent) {
        val uri = intent.data ?: return
        if (uri.scheme != "MeshLink" || uri.host != "relay") return

        val relayUrl = uri.getQueryParameter("url")
        if (relayUrl.isNullOrBlank()) {
            android.widget.Toast.makeText(this, "Missing relay URL", android.widget.Toast.LENGTH_SHORT).show()
            return
        }

        val added = com.MeshLink.android.mesh.UrlRelayPreferences.addRelayUrl(this, relayUrl)
        if (added) {
            meshService.startUrlRelayTransport()
            android.widget.Toast.makeText(this, "Laptop relay connected", android.widget.Toast.LENGTH_SHORT).show()
        } else {
            android.widget.Toast.makeText(this, "Invalid relay URL", android.widget.Toast.LENGTH_SHORT).show()
        }
    }
