...
import com.MeshLink.android.protocol.MessagePadding
import com.MeshLink.android.model.RoutedPacket
import com.MeshLink.android.model.IdentityAnnouncement
import com.MeshLink.android.model.NoisePayload
import com.MeshLink.android.model.NoisePayloadType
...
import com.MeshLink.android.model.RequestSyncPacket
import com.MeshLink.android.sync.GossipSyncManager
import com.MeshLink.android.util.toHexString
import com.MeshLink.android.services.VerificationService
...
class BluetoothMeshService(private val context: Context) {
    private val debugManager by lazy { try { com.MeshLink.android.ui.debug.DebugSettingsManager.getInstance() } catch (e: Exception) { null } }
    
    companion object {
        private const val TAG = "BluetoothMeshService"
        private val MAX_TTL: UByte = com.MeshLink.android.util.AppConstants.MESSAGE_TTL_HOPS
    }
    ...
    private val urlRelayTransport = UrlRelayTransport(context.applicationContext, myPeerID) { routed ->
        packetProcessor.processPacket(routed)
    }
    private lateinit var gossipSyncManager: GossipSyncManager
    // Service-level notification manager for background (no-UI) DMs
    private val serviceNotificationManager = com.MeshLink.android.ui.NotificationManager(
        context.applicationContext,
        androidx.core.app.NotificationManagerCompat.from(context.applicationContext),
        com.MeshLink.android.util.NotificationIntervalManager()
    )
    ...
    init {
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    com.MeshLink.android.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    com.MeshLink.android.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    com.MeshLink.android.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
        ...
        gossipSyncManager.delegate = object : GossipSyncManager.Delegate {
            override fun sendPacket(packet: MeshLinkPacket) {
                broadcastPacket(RoutedPacket(packet))
            }
            override fun sendPacketToPeer(peerID: String, packet: MeshLinkPacket) {
                connectionManager.sendPacketToPeer(peerID, packet)
            }
            override fun signPacketForBroadcast(packet: MeshLinkPacket): MeshLinkPacket {
    ...
    private fun setupDelegates() {
        ...
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                ...
                try { com.MeshLink.android.services.AppStateStore.setPeers(peerIDs) } catch (_: Exception) { }
            ...
            override fun onPeerRemoved(peerID: String) {
                ...
                try { com.MeshLink.android.services.meshgraph.MeshGraphService.getInstance().removePeer(peerID) } catch (_: Exception) { }
        ...
        securityManager.delegate = object : SecurityManagerDelegate {
            ...
            override fun sendHandshakeResponse(peerID: String, response: ByteArray) {
                ...
                broadcastPacket(RoutedPacket(signedPacket))
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun verifySignature(packet: MeshLinkPacket, peerID: String): Boolean {
                return securityManager.verifySignature(packet, peerID)
            }

            override fun getPacketTrust(
                packet: MeshLinkPacket,
                peerID: String
            ): com.MeshLink.android.model.TrustState {
                return securityManager.trustFor(packet, peerID)
            }
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    com.MeshLink.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        com.MeshLink.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
            ...
            // Callbacks
            override fun onMessageReceived(message: MeshLinkMessage) {
                // Always reflect into process-wide store so UI can hydrate after recreation
                try {
                    when {
                        message.isPrivate -> {
                            val peer = message.senderPeerID ?: ""
                            if (peer.isNotEmpty()) com.MeshLink.android.services.AppStateStore.addPrivateMessage(peer, message)
                        }
                        message.channel != null -> {
                            com.MeshLink.android.services.AppStateStore.addChannelMessage(message.channel!!, message)
                        }
                        else -> {
                            com.MeshLink.android.services.AppStateStore.addPublicMessage(message)
                        }
                    }
                } catch (_: Exception) { }
                ...
                // If no UI delegate attached (app closed), show SOS or DM notification via service manager
                if (delegate == null) {
                    val isSos = message.content.trim().startsWith("🆘") || message.meshMessageType == com.MeshLink.android.model.MeshMessageType.SOS
                    if (isSos) {
                        try {
                            val senderPeerID = message.senderPeerID
                            val nick = try { peerManager.getPeerNickname(senderPeerID ?: "") } catch (_: Exception) { null } ?: message.sender
                            serviceNotificationManager.showSosNotification(
                                senderNickname = nick,
                                messageContent = message.content,
                                senderPeerID = senderPeerID
                            )
                        } catch (e: Exception) {
                            Log.e(TAG, "Failed to show background SOS notification: ${e.message}")
                        }
                        ...
                        try {
                            ...
                            if (senderPeerID != null) {
                                ...
                                val preview = com.MeshLink.android.ui.NotificationTextUtils.buildPrivateMessagePreview(message)
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            override fun validatePacketSecurity(packet: MeshLinkPacket, peerID: String): Boolean {
            ...
            override fun handleAnnounce(routed: RoutedPacket) {
                serviceScope.launch {
                    ...
                    if (deviceAddress != null && pid != null) {
                        ...
                        val isDirect = routed.packet.ttl == com.MeshLink.android.util.AppConstants.MESSAGE_TTL_HOPS
            ...
            override fun handleVoiceStream(routed: RoutedPacket) {
                // Ignore our own audio coming back to us. The dedup check upstream keys on the
                // neighbour that delivered the packet, so a relayed copy of our own frame looks like
                // fresh traffic and would otherwise be played back as a delayed echo of ourselves.
                val origin = routed.packet.senderID.joinToString("") { "%02x".format(it) }
                if (origin.equals(myPeerID, ignoreCase = true)) return

                // Voice is now addressed to one peer. Relaying still happens (handled after this),
                // but a device that is merely carrying the packet must not listen to it.
                val recipient = routed.packet.recipientID
                val addressedToUs = recipient == null ||
                    recipient.contentEquals(SpecialRecipients.BROADCAST) ||
                    recipient.toHexString().equals(myPeerID, ignoreCase = true)
                if (!addressedToUs) return

                // Only the party we are actually on a call with gets played, so a third device
                // answering late cannot inject itself into the conversation.
                if (!com.MeshLink.android.features.voice.CallSession.isWith(origin)) return

                // Straight to the player — real-time audio, nothing to parse or persist.
                com.MeshLink.android.features.voice.LiveAudioPlayer
                    .onPacketReceived(routed.packet.payload)
            }

            override fun handleCallEnd(routed: RoutedPacket) {
                val origin = routed.packet.senderID.joinToString("") { "%02x".format(it) }
                if (origin.equals(myPeerID, ignoreCase = true)) return
                serviceScope.launch { delegate?.didReceiveCallEnd(origin) }
            }

            override fun handleCallRequest(routed: RoutedPacket) {
                val peerID = routed.peerID ?: return
                serviceScope.launch { delegate?.didReceiveCallRequest(peerID) }
            }

            override fun handleCallAccept(routed: RoutedPacket) {
                val peerID = routed.peerID ?: return
                serviceScope.launch { delegate?.didReceiveCallAccept(peerID) }
            }

            override fun handleKnowledgeQuery(routed: RoutedPacket) {
                // A peer is asking us to answer on their behalf, because they have no route of their
                // own. Runs on the service scope since it may involve an embedding pass and an HTTP
                // round trip to the Ground Station.
                serviceScope.launch {
                    com.MeshLink.android.features.knowledge.AkashaManager
                        .onMeshQuery(routed.packet.payload)
                }
            }

            override fun handleKnowledgeQueryResult(routed: RoutedPacket) {
                com.MeshLink.android.features.knowledge.AkashaManager
                    .onMeshQueryResult(routed.packet.payload)
            }
            
            override fun handleFragment(packet: MeshLinkPacket): MeshLinkPacket? {
        ...
        override fun onPacketReceived(packet: MeshLinkPacket, peerID: String, device: android.bluetooth.BluetoothDevice?) {
            // Log incoming for debug graphs (do not double-count anywhere else)
            try {
                com.MeshLink.android.ui.debug.DebugSettingsManager.getInstance().logIncoming(
                    packet = packet,
                    fromPeerID = peerID,
                    fromNickname = null,
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.MeshLink.android.ui.debug.DebugSettingsManager.getInstance()
    ...
    fun startServices() {
        ...
        try {
            if (connectionManager.startServices()) {
                ...
                // Knowledge layer needs the mesh up before its relay path can work.
                initKnowledgeLayer()
                ...
                urlRelayTransport.start()
                Log.d(TAG, "URL relay transport start requested")
            } else {
                Log.e(TAG, "Failed to start Bluetooth services cleanly")
            }
        } catch (e: Exception) {
            Log.e(TAG, "Critical exception during startServices: ${e.message}", e)
        }
    }
    ...
    fun stopServices() {
        ...
        // Send leave announcement safely
        try {
            sendLeaveAnnouncement()
        } catch (e: Exception) {
            Log.e(TAG, "Failed to send leave announcement: ${e.message}")
        }
        ...
        serviceScope.launch {
            ...
            try {
                ...
                urlRelayTransport.stop()
                Log.d(TAG, "URL relay transport stopped")
                ...
                Log.e(TAG, "Exception during component shutdown: ${e.message}", e)
            }
    ...
    /**
     * Bring up the knowledge layer and give it the two mesh hooks it needs.
     *
     * Wired here rather than at app start because the relay path can't work before the mesh exists,
     * and because the manager must not import the service — the dependency already runs the other way.
     */
    private fun initKnowledgeLayer() {
        try {
            com.MeshLink.android.features.knowledge.AkashaManager.init(
                context = context,
                sendQuery = { payload -> broadcastKnowledgePacket(MessageType.QUERY, payload) },
                sendResult = { payload -> broadcastKnowledgePacket(MessageType.QUERY_RESULT, payload) },
            )
            com.MeshLink.android.features.knowledge.AkashaManager.setMeshStateProviders(
                peerCount = { peerManager.getActivePeerCount() },
            )
        } catch (e: Exception) {
            Log.w(TAG, "Knowledge layer init failed: ${e.message}")
        }
    }

    /**
     * Send a call-control packet to one peer.
     *
     * Addressed rather than broadcast: an answer concerns only the caller, and an end-of-call concerns
     * only the other party. Broadcasting either would tell the whole mesh about a private call.
     */
    private fun sendCallControl(type: MessageType, toPeerID: String) {
        try {
            val packet = MeshLinkPacket(
                version = 1u,
                type = type.value,
                ...
                payload = ByteArray(0),
                signature = null,
                ttl = com.MeshLink.android.util.AppConstants.MESSAGE_TTL_HOPS,
            )
            broadcastPacket(RoutedPacket(signPacketBeforeBroadcast(packet)))
        } catch (e: Exception) {
            Log.w(TAG, "Failed to send ${type.name} to $toPeerID: ${e.message}")
        }
    }

    /** Tell a caller we are taking their call. */
    fun sendCallAccept(toPeerID: String) = sendCallControl(MessageType.CALL_ACCEPT, toPeerID)

    /** Hang up, decline, or turn away a second answerer. */
    fun sendCallEnd(toPeerID: String) = sendCallControl(MessageType.CALL_END, toPeerID)

    /** Broadcast a knowledge packet, signed like any other so the receiver can attribute it. */
    private fun broadcastKnowledgePacket(type: MessageType, payload: ByteArray) {
        try {
            val packet = MeshLinkPacket(
                version = 1u,
                type = type.value,
            ...
            Log.w(TAG, "Failed to broadcast ${type.name}: ${e.message}")
        }
    }

    internal fun broadcastPacket(routed: RoutedPacket) {
        connectionManager.broadcastPacket(routed)
        urlRelayTransport.send(routed)
    }

    fun startUrlRelayTransport() {
        urlRelayTransport.start()
    }
     ...
     */
    fun sendFileBroadcast(file: com.MeshLink.android.model.MeshLinkFilePacket) {
        try {
     ...
     */
    fun sendFilePrivate(recipientPeerID: String, file: com.MeshLink.android.model.MeshLinkFilePacket) {
        try {
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = com.MeshLink.android.model.NoisePayload(
                            type = com.MeshLink.android.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
                        ...
                        //
                        // Version 2, because v1's payload length field is 16 bits and media routinely
                        // exceeds 64 KB. Sent as v1 this silently wrapped the length and the receiver
                        // decoded a fraction of the packet, which is why private images above ~64 KB
                        // never arrived. Broadcast file transfer already uses v2 for the same reason.
                        val packet = MeshLinkPacket(
                            version = 2u,
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = com.MeshLink.android.model.PrivateMessagePacket(
                    ...
                    val messagePayload = com.MeshLink.android.model.NoisePayload(
                        type = com.MeshLink.android.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { com.MeshLink.android.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = com.MeshLink.android.nostr.GeohashAliasRegistry.snapshot()
                map.containsKey(recipientPeerID)
            } catch (_: Exception) { false }
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(com.MeshLink.android.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }

            try {
                // Avoid duplicate read receipts: check persistent store first
                val seenStore = try { com.MeshLink.android.services.SeenMessageStore.getInstance(context.applicationContext) } catch (_: Exception) { null }
                ...
                val readReceiptPayload = com.MeshLink.android.model.NoisePayload(
                    type = com.MeshLink.android.model.NoisePayloadType.READ_RECEIPT,
    ...
    fun sendBroadcastAnnounce() {
        ...
        serviceScope.launch {
            val nickname = try { com.MeshLink.android.services.NicknameProvider.getNickname(context, myPeerID) } catch (_: Exception) { myPeerID }
            ...
            try {
                ...
                if (directPeers.isNotEmpty()) {
                    val gossip = com.MeshLink.android.services.meshgraph.GossipTLV.encodeNeighbors(directPeers)
            ...
            val announcePacket = MeshLinkPacket(
     ...
     */
    fun getSessionState(peerID: String): com.MeshLink.android.noise.NoiseSession.NoiseSessionState {
     ...
     */
    private fun signPacketBeforeBroadcast(packet: MeshLinkPacket): MeshLinkPacket {
        return try {
            ...
            val withRoute = try {
                ...
                if (rec != null && !rec.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.MeshLink.android.services.meshgraph.RoutePlanner.shortestPath(myPeerID, dest)
...
interface BluetoothMeshDelegate {
    fun didReceiveMessage(message: MeshLinkMessage)
    ...
    // Live voice call. Defaulted so existing delegate implementations don't have to care.
    fun didReceiveCallRequest(peerID: String) {}
    fun didReceiveCallAccept(peerID: String) {}
    fun didReceiveCallEnd(peerID: String) {}
