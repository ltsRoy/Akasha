...
class PacketProcessor(private val myPeerID: String) {
    ...
    private suspend fun handleReceivedPacket(routed: RoutedPacket) {
        ...
        when (messageType) {
            ...
            MessageType.VOICE_STREAM -> handleVoiceStream(routed)
            ...
            MessageType.QUERY -> delegate?.handleKnowledgeQuery(routed)
            MessageType.QUERY_RESULT -> delegate?.handleKnowledgeQueryResult(routed)
    ...
    /**
     * Hand a live audio chunk straight to the player.
     *
     * Deliberately not queued or stored: voice chunks are only useful for the ~40ms they represent,
     * so a late packet is worthless and is better dropped than buffered.
     */
    private suspend fun handleVoiceStream(routed: RoutedPacket) {
        delegate?.handleVoiceStream(routed)
    }
...
interface PacketProcessorDelegate {
    // Security validation
    fun validatePacketSecurity(packet: MeshLinkPacket, peerID: String): Boolean
    ...
    // Knowledge relay
    fun handleKnowledgeQuery(routed: RoutedPacket)
    fun handleKnowledgeQueryResult(routed: RoutedPacket)
