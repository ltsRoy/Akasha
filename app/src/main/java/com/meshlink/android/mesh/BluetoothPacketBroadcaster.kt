...
class BluetoothPacketBroadcaster(
    ...
    companion object {
        ...
        private const val CLEANUP_DELAY = com.MeshLink.android.util.AppConstants.Mesh.BROADCAST_CLEANUP_DELAY_MS
    }
    ...
    private fun logPacketRelay(
        ...
        try {
            ...
            val manager = com.MeshLink.android.ui.debug.DebugSettingsManager.getInstance()
    ...
    /**
     * Voice frames handed to the actor but not yet transmitted.
     *
     * Used to bound how far live audio can fall behind. Four frames is ~160ms of backlog, past which
     * a frame would arrive too late to be worth playing.
     */
    private val pendingVoiceFrames = java.util.concurrent.atomic.AtomicInteger(0)
    private val droppedVoiceFrames = java.util.concurrent.atomic.AtomicLong(0)

    private val MAX_PENDING_VOICE_FRAMES = 4
    ...
    private val broadcasterActor = broadcasterScope.actor<BroadcastRequest>(
        ...
        try {
            for (request in channel) {
                val isVoice = request.routed.packet.type == MessageType.VOICE_STREAM.value
                try {
                    broadcastSinglePacketInternal(request.routed, request.gattServer, request.characteristic)
                } finally {
                    if (isVoice) pendingVoiceFrames.decrementAndGet()
                }
            }
        } finally {
            ...
            if (fragments.size > 1) {
                ...
                val job = connectionScope.launch {
                    ...
                    fragments.forEach { fragment ->
                        ...
                        // 5ms delay between fragments for high-speed transmission (optimized from 20ms)
                        delay(5)
                        if (transferId != null) {
                            sent += 1
    ...
    fun broadcastSinglePacket(
        ...
        // Live audio is dropped rather than queued when the radio falls behind.
        //
        // The actor below is serialized with Channel.UNLIMITED, which is right for text and files —
        // nothing should be lost. It's wrong for voice: at 25 frames a second, any backlog means
        // frames are transmitted progressively later, so the listener hears audio that is seconds
        // stale. A voice frame is only useful for the ~40ms it represents, so shedding it is
        // strictly better than delivering it late.
        if (routed.packet.type == MessageType.VOICE_STREAM.value) {
            if (pendingVoiceFrames.get() >= MAX_PENDING_VOICE_FRAMES) {
                droppedVoiceFrames.incrementAndGet()
                return
            }
            pendingVoiceFrames.incrementAndGet()
        }
        ...
        broadcasterScope.launch {
            try {
                ...
                if (routed.packet.type == MessageType.VOICE_STREAM.value) {
                    pendingVoiceFrames.decrementAndGet()
                }
            }
        }
    }
    ...
    fun getDebugInfo(): String {
        return buildString {
            ...
            appendLine("Voice Frames Pending: ${pendingVoiceFrames.get()}")
            appendLine("Voice Frames Dropped (backlog): ${droppedVoiceFrames.get()}")
