...
import com.MeshLink.android.favorites.FavoritesPersistenceService
import kotlinx.coroutines.flow.StateFlow
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.asStateFlow
import com.MeshLink.android.mesh.BluetoothMeshDelegate
import com.MeshLink.android.mesh.BluetoothMeshService
import com.MeshLink.android.service.MeshServiceHolder
import com.MeshLink.android.model.MeshLinkMessage
import com.MeshLink.android.model.MeshLinkMessageType
import com.MeshLink.android.nostr.NostrIdentityBridge
import com.MeshLink.android.protocol.MeshLinkPacket


import kotlinx.coroutines.launch
import com.MeshLink.android.util.NotificationIntervalManager
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch
import java.util.Date
import kotlin.random.Random
import com.MeshLink.android.services.VerificationService
import com.MeshLink.android.identity.SecureIdentityStateManager
import com.MeshLink.android.noise.NoiseSession
import com.MeshLink.android.nostr.GeohashAliasRegistry
import com.MeshLink.android.util.dataFromHexString
import com.MeshLink.android.util.hexEncodedString
import java.security.MessageDigest

/**
 * Refactored ChatViewModel - Main coordinator for MeshLink functionality
...
class ChatViewModel(
    ...
    companion object {
        ...
        /** Shortest name that still identifies someone on a crowded mesh. */
        private const val MIN_NICKNAME_LENGTH = 2

        /** Auto-generated handles are rejected — accepting them defeats asking in the first place. */
        private val RESERVED_NICKNAME_PATTERN =
            Regex("""^(anon\d*|user\d*|me|test)$""", RegexOption.IGNORE_CASE)
    }
    ...
    private val messageManager = MessageManager(application.applicationContext, state)
    ...
    val messages: StateFlow<List<MeshLinkMessage>> = state.messages
    ...
    val privateChats: StateFlow<Map<String, List<MeshLinkMessage>>> = state.privateChats
    ...
    val channelMessages: StateFlow<Map<String, List<MeshLinkMessage>>> = state.channelMessages
    ...
    init {
        messageManager.loadHistoricalMessages()
    }
    ...
    val selectedLocationChannel: StateFlow<com.MeshLink.android.geohash.ChannelID?> = state.selectedLocationChannel
    ...
    init {
        ...
        viewModelScope.launch {
            try { com.MeshLink.android.services.AppStateStore.peers.collect { peers ->
                state.setConnectedPeers(peers)
                state.setIsConnected(peers.isNotEmpty())
            } } catch (_: Exception) { }
        }
        viewModelScope.launch {
            try { com.MeshLink.android.services.AppStateStore.publicMessages.collect { msgs ->
                // Source of truth is AppStateStore; replace to avoid duplicate keys in LazyColumn
                state.setMessages(msgs)
            } } catch (_: Exception) { }
        }
        viewModelScope.launch {
            try { com.MeshLink.android.services.AppStateStore.privateMessages.collect { byPeer ->
                ...
                try {
                    val seen = com.MeshLink.android.services.SeenMessageStore.getInstance(getApplication())
        ...
        viewModelScope.launch {
            try { com.MeshLink.android.services.AppStateStore.channelMessages.collect { byChannel ->
                // Replace with store snapshot
        ...
        viewModelScope.launch {
            com.MeshLink.android.mesh.TransferProgressManager.events.collect { evt ->
    ...
    private fun loadAndInitialize() {
        ...
        viewModelScope.launch {
            com.MeshLink.android.ui.debug.DebugSettingsManager.getInstance().debugMessages.collect { msgs ->
                if (com.MeshLink.android.ui.debug.DebugSettingsManager.getInstance().verboseLoggingEnabled.value) {
                    ...
                    if (selectedLocation is com.MeshLink.android.geohash.ChannelID.Mesh) {
        ...
        com.MeshLink.android.favorites.FavoritesPersistenceService.initialize(getApplication())
        ...
        try {
            val nostrTransport = com.MeshLink.android.nostr.NostrTransport.getInstance(getApplication())
    ...
    /** Pending live-broadcast invite, surfaced as a dialog on the home screen. */
    val incomingCall: StateFlow<String?> = state.incomingCall

    override fun didReceiveCallRequest(peerID: String) {
        // Joining a broadcast announces itself too, so someone already talking would otherwise be
        // prompted to join the call they are already in.
        if (com.MeshLink.android.features.voice.LiveAudioStreamer.isStreaming.value) {
            Log.d(TAG, "Ignoring call request from $peerID — already in a call")
            return
        }
        state.setIncomingCall(peerID)
    }

    /**
     * A peer has taken our call.
     *
     * The first answer claims it; any later answer is turned away with an explicit end, so a second
     * device can't end up half-joined to a conversation it was never admitted to.
     */
    override fun didReceiveCallAccept(peerID: String) {
        val engaged = com.MeshLink.android.features.voice.CallSession.engage(peerID)
        if (engaged) {
            Log.i(TAG, "Call answered by $peerID")
        } else {
            Log.i(TAG, "Turning away $peerID — already on a call")
            meshService.sendCallEnd(peerID)
        }
    }

    /** The other party hung up, or turned us away. */
    override fun didReceiveCallEnd(peerID: String) {
        if (!com.MeshLink.android.features.voice.CallSession.isWith(peerID) &&
            !com.MeshLink.android.features.voice.CallSession.awaitingAnswer
        ) {
            return
        }
        Log.i(TAG, "Call ended by $peerID")
        com.MeshLink.android.features.voice.CallSession.clear()
        com.MeshLink.android.features.voice.LiveAudioStreamer.stopStreaming()
        com.MeshLink.android.features.voice.LiveAudioPlayer.endCall()
        state.setIncomingCall(null)
    }

    fun rejectCall() {
        val caller = state.incomingCall.value
        state.setIncomingCall(null)
        // Tell the caller rather than leaving them ringing into silence.
        if (caller != null) meshService.sendCallEnd(caller)
    }

    /**
     * Join an in-progress broadcast as a listener.
     *
     * Only opens the receive side: [com.MeshLink.android.features.voice.LiveAudioPlayer.isCallActive]
     * gates playback, so audio is discarded until someone explicitly accepts rather than any packet
     * on the mesh being played automatically.
     */
    /**
     * Take an incoming call.
     *
     * Claims the session for the caller and tells them so, which is what stops anyone else joining.
     * The UI starts the microphone separately, since that needs a permission prompt.
     */
    fun acceptCall() {
        val caller = state.incomingCall.value ?: return
        state.setIncomingCall(null)
        com.MeshLink.android.features.voice.CallSession.engage(caller)
        com.MeshLink.android.features.voice.LiveAudioPlayer.isCallActive = true
        meshService.sendCallAccept(caller)
    }

    // MARK: - Nickname Management

    /**
     * True until the user has picked their own display name.
     *
     * Gates the whole app: on a mesh where identity is just a nickname plus a key fingerprint, a
     * screen full of "anon4271" is unreadable in an emergency, and every peer defaulting to the same
     * pattern makes impersonation trivially easy to overlook.
     */
    private val _needsNicknameSetup = MutableStateFlow(!dataManager.hasChosenNickname())
    ...
    /**
     * Validate a candidate display name, returning an error string or null when acceptable.
     *
     * The restrictions exist for concrete reasons rather than taste:
     *  - '@' and '#' are the mention and geohash-suffix delimiters, so a name containing them breaks
     *    mention parsing and the "nick#abcd" display form.
     *  - Whitespace makes @mentions ambiguous about where the name ends.
     *  - An "anonNNNN" shape is rejected because it defeats the point of asking.
     */
    fun validateNickname(candidate: String): String? {
        val trimmed = candidate.trim()
        return when {
            trimmed.isEmpty() -> "Enter a name so people can recognise you"
            trimmed.length < MIN_NICKNAME_LENGTH -> "At least $MIN_NICKNAME_LENGTH characters"
            trimmed.length > com.MeshLink.android.util.AppConstants.UI.MAX_NICKNAME_LENGTH ->
                "Keep it under ${com.MeshLink.android.util.AppConstants.UI.MAX_NICKNAME_LENGTH + 1} characters"
            trimmed.any { it.isWhitespace() } -> "No spaces — try a dash or underscore"
            trimmed.contains('@') || trimmed.contains('#') -> "Can't contain @ or #"
            RESERVED_NICKNAME_PATTERN.matches(trimmed) -> "Pick something more identifiable than that"
            trimmed.equals("system", ignoreCase = true) -> "That name is reserved"
            else -> null
        }
    }
    ...
    private fun ensureGeohashDMSubscriptionIfNeeded(convKey: String) {
        try {
            ...
            val repo = repoField.get(geohashViewModel) as com.MeshLink.android.nostr.GeohashRepository
            ...
            if (!gh.isNullOrEmpty()) {
                ...
                val subMgr = subMgrField.get(geohashViewModel) as com.MeshLink.android.nostr.NostrSubscriptionManager
                val identity = com.MeshLink.android.nostr.NostrIdentityBridge.deriveIdentity(gh, getApplication())
                val subId = "geo-dm-$gh"
                ...
                if (currentId != subId) {
                    ...
                    subMgr.subscribeGiftWraps(
                        ...
                        handler = { event ->
                            ...
                            val dmHandler = dmHandlerField.get(geohashViewModel) as com.MeshLink.android.nostr.NostrDirectMessageHandler
    ...
    fun openLatestUnreadPrivateChat() {
        try {
            ...
            val openPeer: String = if (targetKey.startsWith("nostr_")) {
                ...
                val canonical = com.MeshLink.android.services.ConversationAliasResolver.resolveCanonicalPeerID(
                    ...
                    nostrPubHexForAlias = { alias -> com.MeshLink.android.nostr.GeohashAliasRegistry.get(alias) },
                    findNoiseKeyForNostr = { key -> com.MeshLink.android.favorites.FavoritesPersistenceService.shared.findNoiseKey(key) }
                )
                canonical ?: targetKey
            }
    ...
    fun sendMessage(originalContent: String) {
        if (originalContent.isEmpty()) return
        
        viewModelScope.launch {
            var content = originalContent
            
            // Check for commands first, no AI needed for commands
            if (content.startsWith("/")) {
                val selectedLocationForCommand = state.selectedLocationChannel.value
                commandProcessor.processCommand(content, meshService, meshService.myPeerID, { messageContent, mentions, channel ->
                    if (selectedLocationForCommand is com.MeshLink.android.geohash.ChannelID.Location) {
            ...
            // Gemini Nano SOS Scan
            val isSOS = com.MeshLink.android.ai.GeminiManager.analyzeMessageForSOS(content)
            if (isSOS && !content.startsWith("🆘")) {
                content = "🆘 $content"
            }
            ...
            if (selectedPeer != null) {
                ...
                selectedPeer = com.MeshLink.android.services.ConversationAliasResolver.resolveCanonicalPeerID(
                ...
                privateChatManager.sendPrivateMessage(
                    ...
                    val router = com.MeshLink.android.services.MessageRouter.getInstance(getApplication(), meshService)
                    router.sendPrivate(messageContent, peerID, recipientNicknameParam, messageId)
                }
            } else {
                // Check if we're in a location channel
                val selectedLocationChannel = state.selectedLocationChannel.value
                if (selectedLocationChannel is com.MeshLink.android.geohash.ChannelID.Location) {
                    ...
                    val type = if (com.MeshLink.android.model.parseLocation(content) != null) {
                        com.MeshLink.android.model.MeshLinkMessageType.Location
                    } else {
                        com.MeshLink.android.model.MeshLinkMessageType.Message
                    }
                    val message = com.MeshLink.android.model.MeshLinkMessage(
    ...
    fun toggleFavorite(peerID: String) {
        ...
        try {
            ...
            if (noiseKey != null) {
                ...
                val identityManager = com.MeshLink.android.identity.SecureIdentityStateManager(getApplication())
                ...
                com.MeshLink.android.favorites.FavoritesPersistenceService.shared.updateFavoriteStatus(
                ...
                try {
                    val myNostr = com.MeshLink.android.nostr.NostrIdentityBridge.getCurrentNostrIdentity(getApplication())
    ...
    private fun updateReactiveStates() {
        ...
        sessionStates.forEach { (peerID, newState) ->
            ...
            if (old != "established" && newState == "established") {
                com.MeshLink.android.services.MessageRouter
    ...
    override fun didReceiveMessage(message: MeshLinkMessage) {
    ...
    fun panicClearAllData() {
        ...
        try {
            com.MeshLink.android.services.SeenMessageStore.getInstance(getApplication()).clear()
        } catch (_: Exception) { }
        ...
        com.MeshLink.android.features.file.FileUtils.clearAllMedia(getApplication())
        ...
        try {
            ...
            try {
                val store = com.MeshLink.android.geohash.GeohashBookmarksStore.getInstance(getApplication())
                store.clearAll()
            } catch (_: Exception) { }
    ...
    fun selectLocationChannel(channel: com.MeshLink.android.geohash.ChannelID) {
