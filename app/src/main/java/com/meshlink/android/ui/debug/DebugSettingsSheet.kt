package com.MeshLink.android.ui.debug
...
import com.MeshLink.android.services.meshgraph.MeshGraphService
        ...
        if (isPresented) {
            ...
            while (true) {
                ...
                val devices = entries.map { (address, isClient, rssi) ->
                    ...
                    com.MeshLink.android.ui.debug.ConnectedDevice(
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        val p = remember(gcsFpr) { com.MeshLink.android.sync.GCSFilter.deriveP(gcsFpr / 100.0) }
                        val nmax = remember(gcsFpr, gcsMaxBytes) { com.MeshLink.android.sync.GCSFilter.estimateMaxElementsForSize(gcsMaxBytes, p) }
                ...
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            Spacer(Modifier.weight(1f))
                            Text("export", color = Color(0xFF007AFF), fontFamily = FontFamily.Monospace, modifier = Modifier.clickable {
                                val logs = manager.exportLogsToString()
                                val sendIntent = android.content.Intent().apply {
                                    action = android.content.Intent.ACTION_SEND
                                    putExtra(android.content.Intent.EXTRA_TEXT, logs)
                                    type = "text/plain"
                                }
                                context.startActivity(android.content.Intent.createChooser(sendIntent, "Export Debug Logs"))
                            })
