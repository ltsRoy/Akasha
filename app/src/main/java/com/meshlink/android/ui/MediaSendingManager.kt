...
import com.MeshLink.android.model.MeshLinkFilePacket
...
class MediaSendingManager(
    ...
    companion object {
        ...
        private const val MAX_FILE_SIZE = com.MeshLink.android.util.AppConstants.Media.MAX_FILE_SIZE_BYTES // 50MB limit
    }
    ...
    fun sendVoiceNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val filePacket = MeshLinkFilePacket(
                fileName = file.name,
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, MeshLinkMessageType.Audio)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, MeshLinkMessageType.Audio)
            }
        } catch (e: Exception) {
    ...
    fun sendImageNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            // Compress against a transport budget rather than a fixed quality. The old path used a
            // 1024px / q60 pass, which landed anywhere between 40 KB and 300 KB depending on the
            // photo — up to ~640 fragments, and with no fragment retransmission a large image was
            // effectively a coin flip. This targets a bounded fragment count instead.
            val contentBytes = com.MeshLink.android.features.media.ImageUtils.compressForMesh(file.readBytes())
            Log.d(TAG, "🗜️ Image prepared for mesh: ${file.length()} -> ${contentBytes.size} bytes")

            val fileSize = contentBytes.size.toLong()
            if (fileSize > MAX_FILE_SIZE) {
                Log.e(TAG, "❌ File too large after compression: $fileSize bytes (max: $MAX_FILE_SIZE)")
                return
            }
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, MeshLinkMessageType.Image)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, MeshLinkMessageType.Image)
            }
        } catch (e: Exception) {
    ...
    fun sendFileNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val mimeType = try { 
                com.MeshLink.android.features.file.FileUtils.getMimeTypeFromExtension(file.name) 
            } catch (_: Exception) { 
                "application/octet-stream" 
            }
            Log.d(TAG, "🏷️ MIME type: $mimeType")

            // Video is not carried over the mesh. Uncompressed clips run to megabytes, which is
            // tens of thousands of 469-byte fragments with no retransmission — the transfer cannot
            // finish. The picker no longer offers video; this refuses anything that arrives by
            // another route (share sheet, generic file picker) instead of failing silently later.
            if (mimeType.lowercase().startsWith("video/")) {
                Log.w(TAG, "❌ Video sending is not supported over the mesh (mime=$mimeType)")
                return
            }

            // Images picked as generic files still need the mesh budget applied, so hand them to
            // the image path rather than shipping a multi-megabyte original.
            if (mimeType.lowercase().startsWith("image/")) {
                Log.d(TAG, "↪️ Routing image file through the compressing image path")
                sendImageNote(toPeerIDOrNull, channelOrNull, filePath)
                return
            }
            ...
            val messageType = when {
                mimeType.lowercase().startsWith("image/") -> MeshLinkMessageType.Image
                mimeType.lowercase().startsWith("audio/") -> MeshLinkMessageType.Audio
                else -> MeshLinkMessageType.File
            }
    ...
    private fun sendPrivateFile(
        toPeerID: String,
        filePacket: MeshLinkFilePacket,
        filePath: String,
        messageType: MeshLinkMessageType
    ) {
        ...
        messageManager.updateMessageDeliveryStatus(
            msg.id,
            com.MeshLink.android.model.DeliveryStatus.PartiallyDelivered(0, 100)
        )
     ...
     */
    fun handleTransferProgressEvent(evt: com.MeshLink.android.mesh.TransferProgressEvent) {
        val msgId = synchronized(transferMessageMap) { transferMessageMap[evt.transferId] }
        if (msgId != null) {
            if (evt.completed) {
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.MeshLink.android.model.DeliveryStatus.Delivered(to = "mesh", at = java.util.Date())
                )
                synchronized(transferMessageMap) {
                ...
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.MeshLink.android.model.DeliveryStatus.PartiallyDelivered(evt.sent, evt.total)
                )
            }
        }
    }
