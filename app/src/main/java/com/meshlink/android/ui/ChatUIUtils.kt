...
fun formatMessageAsAnnotatedString(
    ...
    if (message.sender != "system") {
        ...
        val baseColor = if (isSelf) {
            Color(0xFFD93A34) // accent red — your own voice
        } else {
...
fun formatMessageHeaderAnnotatedString(
    ...
    if (message.sender != "system") {
        val baseColor = if (isSelf) Color(0xFFD93A34) else getPeerColor(message, isDark)
 ...
 */
fun getPeerColor(message: MeshLinkMessage, isDark: Boolean): Color {
    ...
    for ((range, type) in allMatches) {
        ...
        when (type) {
            "mention" -> {
                ...
                val mentionColor = if (isMentionToMe) Color(0xFFD93A34) else baseColor
            ...
            "hashtag" -> {
                ...
                if (type == "geohash") {
                    ...
                    builder.pushStyle(SpanStyle(
                        color = Color(0xFFC9C9C4),                        fontSize = BASE_FONT_SIZE.sp,                        fontWeight = if (isSelf) FontWeight.Bold else FontWeight.SemiBold,                        textDecoration = TextDecoration.Underline
                    ))
