...
import com.MeshLink.android.db.AppDatabase
import com.MeshLink.android.db.MessageEntity

/**
 * Handles all message-related operations including deduplication and organization
 */
class MessageManager(private val context: android.content.Context, private val state: ChatState) {
    ...
    private val MESSAGE_DEDUP_TIMEOUT = com.MeshLink.android.util.AppConstants.UI.MESSAGE_DEDUP_TIMEOUT_MS // 30 seconds
    private val SYSTEM_EVENT_DEDUP_TIMEOUT = com.MeshLink.android.util.AppConstants.UI.SYSTEM_EVENT_DEDUP_TIMEOUT_MS // 5 seconds
    
    // MARK: - Public Message Management
    
    fun addMessage(message: MeshLinkMessage) {
        ...
        try { com.MeshLink.android.services.AppStateStore.addPublicMessage(message) } catch (_: Exception) { }
        
        if (message.sender != "system") {
            scope.launch {
                messageDao.insertMessage(MessageEntity(
                    id = message.id,
                    conversationId = message.channel ?: "public",
    ...
    fun addChannelMessage(channel: String, message: MeshLinkMessage) {
        ...
        try { com.MeshLink.android.services.AppStateStore.addChannelMessage(channel, message) } catch (_: Exception) { }
        ...
        val viewingGeohashChannel = try {
            if (channel.startsWith("geo:")) {
                ...
                selected is com.MeshLink.android.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
            } else false
    ...
    fun addPrivateMessage(peerID: String, message: MeshLinkMessage) {
        ...
        // Reflect into process-wide store
        try { com.MeshLink.android.services.AppStateStore.addPrivateMessage(peerID, message) } catch (_: Exception) { }
    ...
    fun addPrivateMessageNoUnread(peerID: String, message: MeshLinkMessage) {
        val currentPrivateChats = state.getPrivateChatsValue().toMutableMap()
        if (!currentPrivateChats.containsKey(peerID)) {
            currentPrivateChats[peerID] = mutableListOf()
        }
    ...
    fun loadHistoricalMessages() {
        scope.launch {
            try {
                val publicEntities = messageDao.getAllPublicMessages()
                val privateEntities = messageDao.getAllPrivateMessages()
                
                val publicMap = mutableMapOf<String, MutableList<MeshLinkMessage>>()
                val privateMap = mutableMapOf<String, MutableList<MeshLinkMessage>>()
                val generalMessages = mutableListOf<MeshLinkMessage>()
                
                for (entity in publicEntities) {
                    val msg = gson.fromJson(entity.messageJson, MeshLinkMessage::class.java)
                    if (entity.conversationId == "public") {
                        generalMessages.add(msg)
                    } else {
                        if (!publicMap.containsKey(entity.conversationId)) publicMap[entity.conversationId] = mutableListOf()
                        publicMap[entity.conversationId]?.add(msg)
                    }
                }
                
                for (entity in privateEntities) {
                    val msg = gson.fromJson(entity.messageJson, MeshLinkMessage::class.java)
                    if (!privateMap.containsKey(entity.conversationId)) privateMap[entity.conversationId] = mutableListOf()
                    privateMap[entity.conversationId]?.add(msg)
                }
                
                // Now apply to state
                state.setMessages(generalMessages)
                state.setChannelMessages(publicMap)
                state.setPrivateChats(privateMap)
                
            } catch (e: Exception) {
                // ignore parsing errors for historical messages
            }
        }
    }
     ...
     */
    fun generateMessageKey(message: MeshLinkMessage): String {
    ...
    fun updateMessageDeliveryStatus(messageID: String, status: DeliveryStatus) {
        ...
        if (updated) {
            ...
            try { com.MeshLink.android.services.AppStateStore.updatePrivateMessageStatus(messageID, status) } catch (_: Exception) { }
        }
