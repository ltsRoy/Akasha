...
class DataManager(private val context: Context) {
    ...
    companion object {
        ...
        /** Set once the user has typed their own display name. */
        private const val KEY_NICKNAME_CHOSEN = "nickname_user_chosen"

        /** Shape of the placeholder handle generated in [loadNickname], e.g. "anon4271". */
        private val AUTO_NICKNAME_PATTERN = Regex("""^anon\d{3,4}$""", RegexOption.IGNORE_CASE)
    }
    
    private val prefs: SharedPreferences = context.getSharedPreferences("MeshLink_prefs", Context.MODE_PRIVATE)
    ...
    fun loadNickname(): String {
        ...
        return if (savedNickname != null) {
            ...
            // Placeholder only. The mesh needs *some* identity the moment the service starts, well
            // before the user reaches the name prompt, so a temporary anon handle is generated here.
            // It is explicitly not marked as chosen, so the UI still forces a real name.
            val randomNickname = "anon${Random.nextInt(1000, 9999)}"
            prefs.edit().putString("nickname", randomNickname).apply()
            randomNickname
        }
    }

    /**
     * Persist a nickname the user actually typed, and record that they've chosen one.
     *
     * Note the asymmetry with [loadNickname]: only this path sets [KEY_NICKNAME_CHOSEN]. That's what
     * distinguishes "user picked a name" from "we auto-generated a placeholder".
     */
    fun saveNickname(nickname: String) {
        prefs.edit()
            .putString("nickname", nickname)
            .putBoolean(KEY_NICKNAME_CHOSEN, true)
            .apply()
    }

    /**
     * Whether the user has picked their own display name.
     *
     * Includes a migration for installs that predate this flag: an existing nickname that doesn't
     * look auto-generated is treated as chosen, so upgrading users aren't dragged back through a
     * setup screen for a name they already set.
     */
    fun hasChosenNickname(): Boolean {
        if (prefs.getBoolean(KEY_NICKNAME_CHOSEN, false)) return true

        val existing = prefs.getString("nickname", null) ?: return false
        val looksAutoGenerated = AUTO_NICKNAME_PATTERN.matches(existing)
        if (!looksAutoGenerated) {
            // Grandfather it in so the flag is correct from here on.
            prefs.edit().putBoolean(KEY_NICKNAME_CHOSEN, true).apply()
            return true
        }
        return false
    }
